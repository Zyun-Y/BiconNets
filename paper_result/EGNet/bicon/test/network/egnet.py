...
from network.vgg_eg import vgg16
...
class MergeLayer1(nn.Module): # list_k: [[64, 512, 64], [128, 512, 128], [256, 0, 256] ... ]
    def __init__(self, list_k):
        ...
        for ik in list_k:
            ...
            if m ==0:
                ...
                score.append(nn.Conv2d(ik[2], 8, 3, 1, 1))
            m+=1
    ...
    def forward(self, list_x, x_size):
        ...
        # sal_feature: from C6 to C3 output
        # up_sal: deep supervise input of up sampling process
        # edge_feature: the final edge feature
        # up_edge: the final edge output
    ...
    def __init__(self, list_k):
        ...
        for i in list_k[0]:
            ...
            for idx, j in enumerate(list_k[1]):
                ...
                tmp_score.append(nn.Conv2d(i, 8, 3, 1, 1))
        ...
        self.trans, self.up, self.score = nn.ModuleList(trans), nn.ModuleList(up), nn.ModuleList(score)       
        self.final_score = nn.Sequential(nn.Conv2d(list_k[0][0], list_k[0][0], 5, 1, 2), nn.ReLU(inplace=True), nn.Conv2d(list_k[0][0], 8, 3, 1, 1))
        self.relu =nn.ReLU()
...
class TUN_bone(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(up_edge[0].shape, up_sal[0].shape, up_sal_final[0].shape)
