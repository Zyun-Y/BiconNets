...
def conn_loss(c_map,target,con_target):
    ...
    non_edge_pred = pred_mask*(1-edge)


    bce_loss = bce(non_edge_pred.unsqueeze(1), non_edge_tar)
    conn_l = bce(c_map,con_target)
    bimap_l = bce(vote_out,con_target)
    loss =  0.8*conn_l+0.2*bimap_l + edge_loss1

    return pred_mask, loss

def bicon_loss(X, batchs, args):
    bce = nn.BCELoss(reduction='none')

    slc_gt = torch.tensor(batchs['Y']).cuda()
    ctr_gt = torch.tensor(batchs['C']).cuda()
    # print(slc_gt.shape)
    conn_gt = connectivity_matrix(slc_gt.unsqueeze(1).gt(0.5).float())
    ...
    for slc_pred, ctr_pred, weight in zip(X['preds'], X['contour'], args.weights):
        ...
        yconn = connectivity_matrix(ys.unsqueeze(1).gt(0.5).float())
        # print(slc_pred.s)
...
def connectivity_matrix(mask):
    ...
    conn[:,0] = torch.sqrt(mask[:,0]*down_right)
    conn[:,1] = torch.sqrt(mask[:,0]*down)
    conn[:,2] = torch.sqrt(mask[:,0]*down_left)
    conn[:,3] = torch.sqrt(mask[:,0]*right)
    conn[:,4] = torch.sqrt(mask[:,0]*left)
    conn[:,5] = torch.sqrt(mask[:,0]*up_right)
    conn[:,6] = torch.sqrt(mask[:,0]*up)
    conn[:,7] = torch.sqrt(mask[:,0]*up_left)
