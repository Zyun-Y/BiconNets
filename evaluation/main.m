...
for i = 1:numel(algorithms) 
    ...
    for j = 1:numel(datasets)
        ...
        dataset      = datasets{j};
        predpath     = ['/EGNet/bicon/EGNet/' dataset '/pred/'  ];
        maskpath     = ['/EGNet/bicon/EGNet/' dataset '/gt/'  ];
        if ~exist(predpath, 'dir'), continue; end

        names = dir(['/EGNet/bicon/EGNet/' dataset '/pred/*.png']);
        names = {names.name}';
        ...
        for k = 1:numel(names)
            ...
            name_gt = strsplit(name,'pred');
            name_gt = [ char(name_gt(1,1))];
            gtpath = [maskpath name_gt];
            gt = imread(gtpath);
